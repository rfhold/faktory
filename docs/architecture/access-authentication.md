...
Any authenticated MCP principal may retrieve, create, and edit model source and inspect one canonical technical projection per call under the existing `faktory:use` scope. The MVP defines no narrower source-author role. Source access remains MCP-only. MCP tools do not expose arbitrary object keys, Python execution arguments, renderer commands, storage credentials, GLB bytes, or storage-internal metadata. Projection PNG bytes are exposed only through the semantic `model.inspect` image result; browser artifact URLs and browser sessions are not reused for MCP. [`storage-rendering.md`](storage-rendering.md) defines model identity and source semantics.

## Data Safety

Credentials, source text, GLB, SVG, or PNG bytes, session identifiers, authorization codes, tokens, object-store internals, and raw renderer output must not appear in server logs, traces, metrics, protobuf errors, or browser-visible application diagnostics. Authorized MCP `model.get` results are the sole intentional source-text response, and authorized MCP `model.inspect` results are the sole intentional projection-PNG response. Render failures expose only safe bounded summaries.
