...
Adapter and renderer tests must cover exact S3 keys, lowercase project digests, immutable v2 bundle writes, accepted result forms, invalid or absent `result`, GLB validation, bounded execution, restart reconciliation, and conditional view writes. Canonicalization fixtures must cover exact `faktory-project-v2` JSON and final LF, upload-order independence, strict ASCII paths, strict UTF-8 content, BOM and newline normalization, deterministic byte order, duplicate paths, every path rejection, file and byte limits, explicit entrypoint validation, and revision changes for every identity-bearing field.

Exact-key coverage must include project, model-release, rollout, manifest, and every `outputs/{output_id}` artifact path from [`../architecture/storage-rendering.md`](../architecture/storage-rendering.md). Tests must prove that only the primary path has canonical shaded and named-view objects. Tests must reject pre-cutover fixed-key artifacts after migration `0002`.

Project MCP tests must prove the hard removal of singular `source` schemas. Create-schema tests must require only `model_id`, `name`, `files`, and `entrypoint`, with empty defaults for optional `requirements` and `hints`. Get-schema and result tests must accept only `model_id` and return complete metadata plus all desired-project file contents, requirements, locks, and generated `AGENTS.md`. Edit tests must cover one ordered operations array with add, exact patch, delete, rename, entrypoint, dependency, and hint operations; whole-edit revision guards; operation rollback on validation failure; protected AGENTS sections; exact generated index and dependency-guidance bytes; and name-only edits. Exact-patch tests must reject empty, absent, ambiguous, and no-op matches at the step where they occur.
...
Model-release tests must cover stable SemVer parsing, global per-model monotonic publication, byte-identical idempotence, immutable version conflicts, READY revision checks, required package layout, release hashes, and prerelease or build rejection. List and get tests must cover exact metadata, complete closure identities, content-hash file indexes, omitted file bodies, and exact source or docs reads through `model.read`. Namespace tests must cover exact hyphen replacement, the unconditional `m_` prefix, leading digits, keywords, root-package imports, and no `faktory_model` import alias. Tests must prove that only the root entrypoint executes automatically.

Dependency tests must cover exact same-major ranges, 64 direct requirements, highest-compatible direct locks, project revision and release hashes, transitive traversal, own-package imports, and explicit major adoption. They must reject cycles, self-dependencies, undeclared cross-model imports, release or revision conflicts, missing or corrupt releases, 65 unique dependency models, depth nine, and normalized dependency package source above 64 MiB. Deduplication tests must count one shared exact node once and exclude root and non-package files from the source-size sum. Tests must not imply protection against dynamic malicious imports or reject fixed-runtime and standard-library imports.

Rollout tests must prove durable publication intent, same-major minor and patch adoption, no automatic major adoption, deterministic `AGENTS.md` regeneration, per-consumer idempotence, restart resume, concurrent consumer re-evaluation, failed-render advancement, and ordinary last-good preservation. Retry tests must prove that no dependency is re-resolved.

Migration tests must preserve `0001-model-project-bundles` in ordered registration and run `0002-model-dependency-cutover` afterward. They must cover unknown or newer ledgers and old-binary fail-closed behavior. Restart tests must stop after every list, page, delete, empty verification, and ledger write boundary. Tests must prove complete deletion under `models/`, `libraries/`, `system/library-rollouts/`, and obsolete migration `items/` and `backup/` subtrees. They must prove that every completion ledger remains, every target prefix is empty before `0002` completes, and no conversion, backup, alias, rollback, render, or external call occurs.

Post-cutover multipart tests must prove that each successful revision has `outputs.json`. Revisions without it must fail closed. Migration tests own deletion of all pre-cutover revisions.
...
Manifest tests must cover exact `faktory-outputs-v1` bytes, final LF, declared order, finite non-negative facts, one primary, unknown-member rejection, and immutable collision behavior. Access tests must enforce current-success gates, immutable conflicts, recipe-version invalidation, and last-good behavior.

Named-view cache tests must key multipart identity by project revision, primary output ID, view ID, etag, and recipe. They must cover hit, miss, in-flight deduplication, corrupt bytes, worker failure, and immutable conflicts. Revision, primary-output, and etag races before storage or return must produce conflict. Tests must prove that stale cache objects never become selectable. They must reject `output_id` on `view.inspect` and expose no non-primary named-view path.

MCP tests must cover `model.inspect` with omitted and explicit `output_id`, default technical style, and explicit shaded style. Omission must select the primary. Technical requests must select every output. Shaded requests must accept only the primary and reject explicit non-primary IDs as invalid argument. Each success returns one validated semantic PNG block plus output ID, role, and primary metadata without image bytes. Tests must cover fresh and stale revisions, output absence, warning text, recipe metadata, no successful render, and corrupt bytes.
...
Negative tests must prove that credentials, tokens, project or dependency source, AGENTS content, requirements, locks, project docs, GLB, SVG, PNG bytes, object-store internals, and raw renderer output do not leak through logs, signals, errors, protobuf, or browser responses. Exact source-bearing content appears only in authorized MCP model results. Open and release-metadata results must omit indexed bodies. Bounded tools must not exceed their content and result caps. One PNG appears only in authorized `model.inspect` or `view.inspect` results. Worker, rollout, and migration logs and errors must follow the same boundary. Browser tests enforce the Faro boundary in [`../architecture/observability.md`](../architecture/observability.md).
