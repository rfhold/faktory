...
The repository contains the MVP server, CadQuery renderer, Solid SPA, MCP tools, protocol bindings, and a production container image. It also contains an intentionally unauthenticated, loopback-only Garage and PostgreSQL local Compose environment plus preview deployment declarations. Production uses Authentik browser sessions and hosted MCP OAuth. The implementation and local tests exist; no production infrastructure or external Authentik, Ceph, PostgreSQL, Kubernetes, or cloud resource has been configured or deployed from this repository.
