...
Any authenticated MCP principal may retrieve, create, and edit model source. The MVP defines no narrower source-author role. Source access remains MCP-only. MCP tools do not expose arbitrary object keys, Python execution arguments, renderer commands, storage credentials, geometry bytes, or storage-internal metadata. [`storage-rendering.md`](storage-rendering.md) defines model identity and source semantics.

## Data Safety

Credentials, source text, GLB or SVG bytes, session identifiers, authorization codes, tokens, object-store internals, and raw renderer output must not appear in server logs, traces, metrics, protobuf errors, or browser-visible application diagnostics. Authorized MCP `model.get` results are the sole intentional source-text response. Render failures expose only safe bounded summaries.
