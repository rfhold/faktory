...
Project files, library source, guidance, docs, geometry, preview, and projection bytes never travel in protobuf messages. Metadata facts and `updated_at` travel in complete `Model` records. Projection PNGs travel only in semantic MCP image results, never through browser artifact URLs.
