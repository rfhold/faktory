...
This document defines the implemented first-milestone multipart design-bundle contract. Direct CadQuery values remain supported through the documented one-output normalization. Repository state does not prove a preview or production deployment.
...
Output IDs, roles, order, and primary selection come from evaluated source. They do not alter canonical project revision identity. Project files, entrypoint, direct requirements, and exact model-release locks remain the complete identity inputs. Source must produce the same design bundle for the same revision and lock closure. Nondeterministic artifact bytes can cause an immutable-write conflict and fail the render.
...
The model-dependency cutover deletes every revision created before the v2 project contract. Every post-cutover successful revision contains `outputs.json`. The multipart manifest and output semantics otherwise remain unchanged.

## Source-Level Design Semantics

Projects and released model packages can expose ordinary Python helpers for constraints, interfaces, mating, placement, and output construction. These APIs remain source-level behavior under exact project revisions and model-release locks. Faktory does not persist or interpret them as structured server constraints.
...
Concrete hardware definitions belong in exact immutable model releases. A change that affects physical geometry, fit, clearance, or mating interfaces requires a new major release. Minor and patch releases must remain physically and API compatible with every earlier release in that major. Faktory cannot infer compatibility from Python source; the publisher must choose the correct major version. Consumers adopt a new major through an explicit `dependencies.set` range change.
