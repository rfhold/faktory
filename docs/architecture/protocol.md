...
The repository implements the multipart protobuf, route, and model-dependency protocol behavior below. Repository state does not prove a preview or production deployment.
...
MCP tools remain outside the protobuf service contract. Bulk `model.create`, `model.get`, and `model.edit` use the v2 schemas in [`model-projects-dependencies.md`](model-projects-dependencies.md). Agent-oriented `model.open`, `model.read`, `model.glob`, `model.grep`, and `model.apply_patch` operate over those canonical revisions. `model.release.list`, `model.release.publish`, and `model.release.get` manage immutable model releases. Exact release source and docs use `model.read` with the release's project revision. All tool input objects reject unknown fields, and singular `source` inputs and outputs remain absent.

`model.inspect` accepts `model_id`, one closed `projection` value, optional `render_style`, and optional `output_id`. The style accepts `technical` or `shaded` and defaults to `technical`. An omitted `output_id` selects the primary output. Technical inspection accepts every current-successful output. Shaded inspection accepts only the primary; an explicit non-primary shaded request returns invalid argument. A missing output in the selected current-successful revision returns not found.

The tool returns text, exactly one base64 `image/png` block, structured metadata without image bytes, and `isError: false`. Metadata includes model identity, selected output ID, role, primary status, 640x480 dimensions, desired and rendered revisions, desired-revision render state, MIME type, and `stale`. Shaded metadata also includes `style` and recipe `three-v2`. `stale` equals true when desired and rendered revisions differ. A stale result returns the retained last-good output image with explicit warning text.

`view.inspect` accepts exactly `model_id` and `view_id`. It returns one primary-output shaded 640x480 PNG for the exact saved camera. Its metadata also includes the primary output ID and role. The tool rechecks the successful revision, primary output identity, and view etag before storage and before return. A concurrent source success or view mutation returns a safe conflict instead of mismatched bytes. Per-output named views and an `output_id` input are excluded.

A model without a successful render returns a safe not-found error. Corrupt stored images return invalid state. Worker absence, timeout, crash, or saturation returns a retry-enabled unavailable error for on-demand work. Invalid input returns invalid argument. Non-retryable identity races return conflict. [`storage-rendering.md`](storage-rendering.md) defines model IDs, project changes, and render caches. Protobuf model records remain metadata-only. The model-dependency cutover adds no fields; the multipart contract adds only output summaries.

`Model.desired_source_revision` retains its protobuf name but contains the lowercase canonical project SHA-256 selected by the latest accepted project creation, project edit, or compatible model-release rollout. `current_successful_source_revision` identifies the project revision whose output manifest, artifacts, and facts remain available. They differ during a render and after a failed replacement, except that an explicit same-project rerender keeps them equal while work is pending. `render_state` describes work for the desired revision; `render_error` is empty except for a safe, bounded failure summary.

`Model.current_successful_facts` remains the primary output's facts for wire compatibility. The field is absent when no successful geometry exists. The schema adds `OutputRole` with `OUTPUT_ROLE_UNSPECIFIED`, `OUTPUT_ROLE_ASSEMBLY`, `OUTPUT_ROLE_PART`, and `OUTPUT_ROLE_TOOL`. A populated summary never uses `OUTPUT_ROLE_UNSPECIFIED`. `ModelOutputSummary` contains `output_id`, `role`, `primary`, and `facts`. A repeated `Model.current_successful_outputs` field contains every output in manifest order. It is empty when no successful geometry exists. Exactly one summary is primary, and its facts equal `current_successful_facts`.

Migration `0002-model-dependency-cutover` deletes revisions without an output manifest. Post-cutover successful revisions always provide explicit output summaries. `Model.updated_at` retains its current mutation semantics. List, get, snapshot, and change responses carry these fields in complete `Model` records.
