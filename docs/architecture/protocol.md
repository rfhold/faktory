...
The multipart protobuf and route additions below define approved behavior under implementation. Current generated bindings and executable routes can remain singular on this branch. This status does not imply deployment.
...
Revisions without an output manifest produce one synthetic summary with output ID `primary`, role `OUTPUT_ROLE_ASSEMBLY`, primary true, and the legacy facts. [`design-bundles.md`](design-bundles.md) defines why legacy role inference differs from source normalization. `Model.updated_at` retains its current mutation semantics. List, get, snapshot, and change responses carry these fields in complete `Model` records.
